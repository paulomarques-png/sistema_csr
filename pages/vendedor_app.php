<?php
// ============================================================
// pages/vendedor_app.php — App Mobile do Vendedor (Parte 10)
// Acesso exclusivo para perfil: vendedor
// ============================================================
...
$stmtU = $pdo->prepare("
    SELECT vendedor_id FROM usuarios WHERE id = :uid LIMIT 1
");
$stmtU->execute([':uid' => $_SESSION['usuario_id']]);
$vendedorId = (int)($stmtU->fetchColumn() ?? 0);

// Sem vínculo: exibe aviso amigável
if ($vendedorId <= 0) {
...
function saldoDia(PDO $pdo, int $vid, string $data): array
{
    ...
    $saidas = (int)$pdo->prepare("
        SELECT COALESCE(SUM(quantidade),0) FROM reg_saidas
        WHERE vendedor_id = :vid AND data = :data
    ")->execute($p) ? $pdo->prepare("
        SELECT COALESCE(SUM(quantidade),0) FROM reg_saidas
        WHERE vendedor_id = :vid AND data = :data
    ")->execute($p) : 0;

    // Re-executa corretamente (acima foi só para evitar inline)
    $st = $pdo->prepare("SELECT COALESCE(SUM(quantidade),0) FROM reg_saidas WHERE vendedor_id=:vid AND data=:data");
    $st->execute($p); $saidas = (int)$st->fetchColumn();

    $st = $pdo->prepare("SELECT COALESCE(SUM(quantidade),0) FROM reg_retornos WHERE vendedor_id=:vid AND data=:data");
    $st->execute($p); $retornos = (int)$st->fetchColumn();

    $st = $pdo->prepare("SELECT COALESCE(SUM(quantidade),0) FROM reg_vendas WHERE vendedor_id=:vid AND data=:data");
...
function saldoPorProduto(PDO $pdo, int $vid, string $data): array
{
    ...
    foreach ($prods as $cod => &$p2) {
        $p2['saldo'] = $p2['saida'] - $p2['retorno'] - $p2['vendido'];
    }
    unset($p2);

    // Ordena: com saldo pendente primeiro
...
function historico7Dias(PDO $pdo, int $vid): array
{
    ...
    $st = $pdo->prepare("SELECT data, SUM(quantidade) AS q FROM reg_saidas WHERE vendedor_id=:vid AND data>=:ini GROUP BY data");
    $st->execute($p);
    foreach ($st->fetchAll() as $r) { if (isset($dias[$r['data']])) { $dias[$r['data']]['saidas']=(int)$r['q']; $dias[$r['data']]['tem_movimento']=true; } }

    $st = $pdo->prepare("SELECT data, SUM(quantidade) AS q FROM reg_retornos WHERE vendedor_id=:vid AND data>=:ini GROUP BY data");
    $st->execute($p);
    foreach ($st->fetchAll() as $r) { if (isset($dias[$r['data']])) { $dias[$r['data']]['retornos']=(int)$r['q']; $dias[$r['data']]['tem_movimento']=true; } }

    $st = $pdo->prepare("SELECT data, SUM(quantidade) AS q FROM reg_vendas WHERE vendedor_id=:vid AND data>=:ini GROUP BY data");
    $st->execute($p);
    foreach ($st->fetchAll() as $r) { if (isset($dias[$r['data']])) { $dias[$r['data']]['vendas']=(int)$r['q']; $dias[$r['data']]['tem_movimento']=true; } }

    foreach ($dias as &$d) {
        $d['saldo'] = $d['saidas'] - $d['retornos'] - $d['vendas'];
    }
    unset($d);
...
// ── QR Codes pendentes para este vendedor ────────────────────────
function qrsPendentes(PDO $pdo, int $vid): array
{
    $st = $pdo->prepare("
...
$qrsPend  = qrsPendentes($pdo, $vendedorId);
...
<main class="vapp-main">
    ...
    <?php /* ── QR Codes aguardando confirmação ─────────────────── */ ?>
    <?php foreach ($qrsPend as $qr): ?>
    <div class="vapp-qr-alerta">
        <div class="vapp-qr-alerta-icone">📱</div>
        <div class="vapp-qr-alerta-texto">
            <strong>QR Code aguardando sua confirmação!</strong><br>
            <small>
                <?= $qr['tipo'] === 'saida' ? '📤 Saída' : '📥 Retorno' ?>
                — <?= formatarData($qr['data_ref']) ?>
                — Expira: <?= formatarDataHora($qr['expira_em']) ?>
            </small>
        </div>
        <a href="<?= BASE_URL ?>/qr/confirmar.php?token=<?= esc($qr['token']) ?>"
           class="vapp-qr-alerta-btn">
            Confirmar →
        </a>
    </div>
    <?php endforeach; ?>

    <?php /* ── Cards de saldo do dia ──────────────────────────── */ ?>
    <div class="vapp-saldo-header">
    ...
    <?php
        ...
        $textoSaldo = $resumo['saldo'] === 0 ? 'Tudo zerado!' : ($resumo['saldo'] > 0 ? 'Saldo em aberto' : 'Verificar — saldo negativo');
    ?>
